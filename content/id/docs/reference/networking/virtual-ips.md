---
title: IP Virtual dan Proksi Service
content_type: reference
weight: 50
---

<!-- overview -->
Setiap {{< glossary_tooltip term_id="node" text="node" >}} di 
{{< glossary_tooltip term_id="cluster" text="klaster" >}} Kubernetes menjalankan 
[kube-proxy](/docs/reference/command-line-tools-reference/kube-proxy/)
(kecuali Anda sudah menjalankan komponen alternatif lain sebagai pengganti `kube-proxy`).

Komponen `kube-proxy` bertanggung jawab untuk mengimplementasikan mekanisme _virtual IP_
untuk {{< glossary_tooltip term_id="service" text="Service">}}
dengan `type` selain
[`ExternalName`](/docs/concepts/services-networking/service/#externalname). Setiap kube-proxy yang dibuat memantau watches {{< glossary_tooltip
term_id="control-plane" text="panel kontrol" >}}  Kubernetes untuk menambah atau menghapus {{< glossary_tooltip
term_id="object" text="objek" >}} Service dan EndpointSlice. Untuk setiap Service, kube-proxy
memanggil API terkait (tergantung dari mode kube-proxy yang digunakan) untuk mengonfigurasi
 *node* dalam menangkap *traffic* dari `clusterIP` dan `port` suatu Service,
dan melakukan *redirect* *traffic* tersebut ke salah satu *endpoint* suatu Service
(biasanya Pod, tapi mungkin juga alamat IP yang diberikan pengguna secara acak). *Control loop* memastikan bahwa setiap aturan di setiap *node* benar - benar sinkron secara akurat dengan status Service dan EndpointSlice yang dipantau oleh API Server.

{{< figure src="/images/docs/services-iptables-overview.svg" title="Mekanisme IP Virtual untuk Service adalah menggunakan mode iptables" class="diagram-medium" >}}

Pertanyaan yang seringkali muncul adalah kenapa Kubernetes mengandalkan *proxying* untuk meneruskan *traffic* yang data ke *backend*. Bagaimana dengan pendekatan lain? sebagai contoh, apakah mungkin mengonfigurasi baris DNS yang mempunyai nilai A (atau AAAA untuk IPv6) yang lebih dari satu serta mengandalkan *round-robin* untuk resolusi nama?

Terdapat beberapa alasan dalam penggunaan *proxying* untuk Service:

* Terdapat sejarah yang pajang dari implementasi DNS yang tidak menghiraukan baris TTLs, dan menyimpan sementara hasil dari pencarian nama yang sudah kadaluwarsa.
* Beberapa aplikasi melakukan pencarian DNS sekali saja dan menyimpan hasilnya sementara tanpa batas waktu
* Bahkan ketika aplikasi atau pustaka melakukan resolusi ulang, TTLs yang rendah atau nol pada baris DNS dapat berakibat beban tinggi pada DNS yang nantinya akan sulit ditangani

Selanjutnya di halaman ini, Anda dapat membaca tentang cara kerja berbagai implementasi `kube-proxy`. Ringkasnya, sebagai catatan, ketika menjalankan `kube-proxy`, *rule* tingkat kernel dapat dimodifikasi (sebagai contoh, *rule* iptable mungkin dibuat), yang mungkin tidak akan dibersihkan di beberapa kasus sampai akhirnya di-*boot* ulang. Oleh karena itu, menjalankan `kube-proxy` adalah sesuatu yang hanya boleh dijalankan oleh administrator yang mengerti konsekuensi dari melakukan *proxying* jaringan Service di komputer dengan memiliki hak akses tinggi dan di tingkat arsitektur yang lebih rendah. Meskipun `kube-proxy` mendukung fungsi *cleanup*, fungsi ini belum menjadi fitur resmi dan oleh karena itu hanya tersedia untuk digunakan apa adanya.

<a id="example"></a>
Beberapa detail di referensi ini menunjukkan sebuah contoh: *backend*
{{< glossary_tooltip term_id="pod" text="Pods" >}} untuk aplikasi pengolahan citra yang *stateless*, berjalan dengan tiga replika. Replika pods tersebut ada masanya&mdash;*frontend* tidak memperdulikan *backend* mana yang mereka gunakan. Selama *pods* aktual yang menyusun *backend* mungkin terganti, *frontend* tidak perlu tahu hal tersebut, atau dengan sengaja memantau setiap perubahan pods di kumpulan *backend* oleh mereka sendiri.

<!-- body -->

## Mode proxy

`kube-proxy` dapat berjalan dengan berbagai mode, tergantung konfigurasinya.

Pada *node* Linux, mode yang tersedia untuk `kube-proxy` antara lain:

[`iptables`](#proxy-mode-iptables)
: Mode dimana `kube-proxy` mengonfigurasi aturan pengiriman paket menggunakan iptables.

[`ipvs`](#proxy-mode-ipvs)
: mode dimana `kube-proxy` mengonfigurasi aturan pengiriman paket menggunakan ipvs.

[`nftables`](#proxy-mode-nftables)
: mocd dimana `kube-proxy` mengonfiguras aturan pengiriman paket menggunakan nftables.

Terdapat juga satu - satunya mode `kube-proxy` yang tersedia di Windows:

[`kernelspace`](#proxy-mode-kernelspace)
: mode dimana `kube-proxy` mengonfigurasi aturan pengiriman paket di kernel Windows
...
Untuk mempelajari Service lebih lanjut,
silahkan baca [Menghubungkan Aplikasi dengan Service](/docs/tutorials/services/connect-applications-service/).

Anda juga dapat:

* Membaca tentang konsep [Service](/docs/concepts/services-networking/service/)
* Membaca tentang konsep [Ingress](/docs/concepts/services-networking/ingress/)
* Membaca [Referensi API](/docs/reference/kubernetes-api/service-resources/service-v1/) untuk API Service
